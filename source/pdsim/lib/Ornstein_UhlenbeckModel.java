...
public class Ornstein_UhlenbeckModel  extends ProbabilityCorContCharModel {
	...
	public void setDefaults(Taxa taxa, MContinuousDistribution data, int ic){
		...
		//MesquiteCommand setMeanVar=new MesquiteCommand("setmean_var",this);
	...
	public void initialize(Tree tree){
		if (tree!=null) {
			...
			System.out.println("(span,numberOfTerminals):("+span+","+tree.numberOfTerminalsInClade(tree.getRoot())+")");
			
			peak_vel=(mean_tip_peak-root_peak)/(mean_height);
			nord_var=trait_var/nordheim;
			trait_vel=(mean_tip_state-root_state)/(mean_height);
			System.out.println("(peak,nordheim,averagehieght):("+trait_vel+","+nordheim+","+mean_height+")");
	...
	public double[] evolveMultiState (double beginStates[], CorTrait rnd, double BranchLength, int node, int trait, boolean bound){
...
//		System.out.println("peakSate:"+peakState);
        step_peak=super.getchangevelocity(BranchLength,rng.nextGaussian(),peak_var,peak_vel);
		step_trait_vel=super.getchangevelocity(BranchLength,rnd.get_num(0,trait),nord_var,trait_vel);
		//TODO decay cannot = 0;	
		double temp;
		if(decay!=0){
			temp=traitState-peakState-(step_trait_vel+step_peak)/decay;
		}
		else temp=0;
        ...
        if (decay*BranchLength>80){
        	...
        	step=temp*Math.exp(-1*decay*BranchLength) + step_trait_vel*BranchLength - temp;
        //System.out.println("step.");
		if (bound){
			System.out.println("checking bounds!");
			boolean do_over=false;
			traitState=super.bound(traitState,step,bounding_type,lower_bound,upper_bound,do_over);
			if(do_over){
				System.out.println("Boing!");
				evolveMultiState(beginStates,  rnd,  BranchLength,  node,  trait,  bound);
			}
		}
		else traitState=traitState+step;
   	...
   	public Object doCommand(String commandName, String arguments, CommandChecker checker){
		...
		if (!MesquiteDouble.isCombinable(newtrait_var)) {
			newtrait_var= MesquiteDouble.queryDouble(MesquiteTrunk.mesquiteTrunk.containerOfModule(), "Set trait_var", "trait_var of change:", a);
	 		...
	 		if (newvar < upper_bound && newvar!=a && MesquiteDouble.isCombinable(newvar)) {
	 			lower_bound=newvar;
	 		}
	 		else if (newvar >= upper_bound){
	 			System.out.println("assignment failure:lower bound must be less than upper bound");
	 			MesquiteInteger buttonPressed = new MesquiteInteger(1);
 				ExtensibleDialog boo=new ExtensibleDialog(MesquiteTrunk.mesquiteTrunk.containerOfModule(),"boo!",buttonPressed);
			...
			pos.setValue(0);
	 		...
	 		if (newvar > lower_bound && newvar!=a && MesquiteDouble.isCombinable(newvar)) {
	 			upper_bound=newvar;
	 		}
	 		else if (newvar <= upper_bound){
	 	...
	 	else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setpeak_var")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
	 		double a = peak_var;
	 		if (!MesquiteDouble.isCombinable(newvar)) {
		...
		else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setnord_var")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
		...
		else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setpeak_vel")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
	 		double a = peak_vel;
	 		if (!MesquiteDouble.isCombinable(newvar)) {
		...
		else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setdecay")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
	 		double a = decay;
	 		if (!MesquiteDouble.isCombinable(newvar)) {
	 	...
	 	else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setMeanTipPeak")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
	 	...
	 	else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setRootPeak")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
	 		double a = root_peak;
	 		if (!MesquiteDouble.isCombinable(newvar)) {
	 	...
	 	else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setRootState")) {
	 		pos.setValue(0);
	 		double newvar = MesquiteDouble.fromString(arguments, pos);
	 		double a = root_state;
	 		if (!MesquiteDouble.isCombinable(newvar)) {
	...
	public CharacterModel cloneModelWithMotherLink(CharacterModel formerClone){
		Ornstein_UhlenbeckModel bmm = new Ornstein_UhlenbeckModel(name, getStateClass());
	...
	public String getExplanation (){
		return "A stochastic model in which expected change is distributed normally with mean 0, and variance proportional to the rate. Branch Length is ignored.  The current rate is " + MesquiteDouble.toString(trait_var);
	}
