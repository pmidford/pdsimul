...
//	int numOfGroups;
	...
	public Results subfinish(CoVarMatrixModel model, String name, String description, MesquiteInteger stringPos, int format){
		pdanovaResults results=new pdanovaResults(name, Results.class, model);
		initialized=true;
		editing=results;
...
//		getOwnerModule().getFileCoordinator();
		...
		try {
			// TODO Auto-generated catch block
				DataFile.MakeDataFileWithDialog(editing, "TODO:FIXME", this);
		}
		catch (Exception em) {
			em.printStackTrace();
		}
		...
		for(int x=0;x<editing.numOfTraits;x++){
			str[x]=editing.model.getTraitName(x); //(x);
		}
		...
		//	System.out.println("158");										
			testStat=PDAnova.getFStat(testGroup);
		...
		nodeNameVector=PDAnova.getTipNameVector(nodeNameVector, editing.model.getTree(), editing.model.getTree().getRoot(), 0);
