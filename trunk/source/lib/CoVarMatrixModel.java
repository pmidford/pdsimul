...
import mesquite.minimal.BasicFileCoordinator.*;
...
public class CoVarMatrixModel  extends ProbabilityContCharModel implements CorTraitModel, DataSource {
		...
		MesquiteBoolean showCholesky=new MesquiteBoolean(false);

		String treename=null, matrixname=null;
		int matrixNum=0, treeNum=0;
...
//		}
		...
		public void setToList(){
			...
			for(int x=0;x<num_of_traits;x++){
				...
				if(!name.equalsIgnoreCase("unassigned"))
	...
	//		System.out.println("hi3");
		...
		public String getNexusSpecification(){
			...
			specs+="\n";
//			specs+= //&& var2.equalsIgnoreCase("matrixname""
			...
			Results[] results;
			if(rm!=null)
				System.out.println("resultsTask!=null");
			else
				System.out.println("resultsTask==null");
			...
			for (int x=0;x<results.length;x++){
				specs+="\tRESULT ";
				specs+=results[x].toString()+"\n";
			}
			specs+="ENDCOVARMATRIX";
...
//		System.out.println("set 3");*/
		...
		public Object doCommand(String commandName, String arguments, CommandChecker checker) {
			if (checker.compare(this.getClass(), "Displays the cholesky for the current covariance matrix", "[number of traits]", commandName, "toggelCholesky")) {
				System.out.println("setCholesky");
					traits.get_cholesky();
				if(showCholesky.getValue())
					showCholesky.setValue(false);
				else showCholesky.setValue(true);
				notifyListeners(this, new Notification(MesquiteListener.UNKNOWN), DataSource.class, false);
				for(int x=0;x<num_of_traits;x++){
					for(int y=0;y<num_of_traits;y++){	
			...
			else if (checker.compare(this.getClass(), "Sets model to control trait", "[number of trait to set]", commandName, "setModel")) {
		...
		/** Returns correlation between two state. */
...
//						return false;f//			}
		...
		/*public static CharacterModel getNewModel(MesquiteModule m, ModelCompatibilityInfo mci, String explanation){
			Listable[] models = m.getProject().getCharacterModels(mci, null);
			for(int x=0;x<models.length;x++){
				if (((ProbabilityCorContCharModel)models[x]).isBuiltIn())
					models[x].
			}
