...
public class PDSIM_BrownianModel  extends ProbabilityCorContCharModel {
	...
	public void setDefaults(Taxa taxa, MContinuousDistribution data, int ic){
		...
		MesquiteCommand setMeanTipState=new MesquiteCommand("setmeanTipState",this);

		MesquiteCommand setRootState=new MesquiteCommand("setroot_state",this);
			//MesquiteCommand setMeanVar=new MesquiteCommand("setmean_var",this);
		...
		setMeanTipState.doIt(MesquiteDouble.toString(PDAnova.getMean(states)));
	...
	public double[] evolveMultiState (double beginState[], CorTrait rnd, double BranchLength, int node, int trait, boolean bound){
		...
		System.out.println("(node,trait:num)"+node+","+trait+":"+rnd.get_num(0,trait));
		...
		if (bound){
			...
			if(do_over.getValue())evolveMultiState(beginState,  rnd,  BranchLength,  node,  0,  bound);
		}
	...
	public double evolveState (double beginState, Tree tree, int node){
		...
		else BranchLength=tree.getBranchLength(node, 1);
		
		//p.nordvar[1]:=sqrt(p.selvariance[1])/p.nordheim;		
		step_mean_vel=super.getchangevelocity(BranchLength,rng.nextGaussian(),mean_var,mean_vel);
		step_trait_vel=super.getchangevelocity(BranchLength,rng.nextGaussian(),nord_var,trait_vel);				
		step=BranchLength*(step_mean_vel+step_trait_vel);   //pre-1.05 this failed to take the sqrt!!!!
		System.out.println("checing bounds? "+use_bounds+" "+this.getName()+" "+this.getNexusSpecification()); //toString());
		if (use_bounds){
			System.out.println("checing bounds!");
			MesquiteBoolean do_over=new MesquiteBoolean(false);
		//	do_over.setValue(false);
			beginState=super.bound(beginState,step,bounding_type,lower_bound,upper_bound,do_over);
			if(do_over.getValue())evolveState(beginState,  tree, node);
		}
		else beginState=beginState+step;
		return beginState;
	}
 	/*.................................................................................................................*/
	...
	public void initialize(Tree tree){
			...
			System.out.println("(mean,nord):("+mean_vel+","+nord_var+")");
			initialized=true;
		}
		else{
   	...
   	public Object doCommand(String commandName, String arguments, CommandChecker checker){
		...
		if (!MesquiteDouble.isCombinable(newtrait_var)) {
			newtrait_var= MesquiteDouble.queryDouble(MesquiteTrunk.mesquiteTrunk.containerOfModule(), "Set trait_var", "trait_var of change:", a);
	 		...
	 		if ( newtrait_var!=a && MesquiteDouble.isCombinable(newtrait_var)) {
	 			trait_var=newtrait_var;
	 		}
	 	}
	 	else if (checker.compare(this.getClass(), "Sets the instantaneous trait_var of change in the model", "[trait_var of change; must be > 0]", commandName, "setmean_var")) {
	 		pos.setValue(0);
...
/*   	public void setlower_bound(double x){
	...
	public CharacterModel cloneModelWithMotherLink(CharacterModel formerClone){
		System.out.println("cloneModelWithMotherLink");
	...
	public void copyToClone(CharacterModel md){
		System.out.println("copyToClone");
	...
	public String getExplanation (){
		return "A stochastic model in which expected change is distributed normally with mean "+(meanTipState-root_state)+", and variance of tip states is "+trait_var;
	}
