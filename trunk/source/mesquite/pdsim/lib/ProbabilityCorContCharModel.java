...
public abstract class ProbabilityCorContCharModel  extends ProbabilityContCharModel {
	...
	public double bound (double beginState, double step, String bounding_type, double lower_bound, double upper_bound, boolean do_over){
		double endState=beginState+step;
		if(bounding_type=="Truncation"){
		...
		else if (bounding_type=="Hard_Bounce"){
			if(endState<lower_bound){
	...
	public static double findspan(Tree tree, int node, boolean use_length, double hieght, double span){
		...
		for (int daughter = tree.firstDaughterOfNode(node); tree.nodeExists(daughter); daughter = tree.nextSisterOfNode(daughter)){
			span=findspan(tree, daughter, use_length, hieght, span);
		}
	...
	public static double findkterm(Tree tree, int node, boolean use_length, double kterm){
		int tips;
		...
		for (int daughter = tree.firstDaughterOfNode(node); tree.nodeExists(daughter); daughter = tree.nextSisterOfNode(daughter)){
			kterm=findkterm(tree, daughter, use_length, kterm);
		}		
