...
//	metaGroup= new Vector();
	...
	F = new double[numOfGroups][numOfRuns];
	...
	for(int x=0;x<numOfRuns;x++){
		...
		for(int y=0;y<numOfTraits;y++){
			...
			F[y][x]=PDAnova.getFStat(DataVector[y], numOfGroups);
		}
	}
}
	public void dispose(){
	 ...
	 */	static public double getFStat(Vector[] Group, int numOfGroups){
...
class PDAnovaMenu extends ExtensibleDialog implements ItemListener, ActionListener {
	...
	public void itemStateChanged(ItemEvent e){
		...
		for (int y=0;y<maxGroupNum;y++){
			if (state[xloc+page*maxDisplay][y]){
				state[xloc+page*maxDisplay][y]=false;
				mx[xloc][y].setState(false);
			}
		}
		temp.setState(true);
		state[xloc+page*maxDisplay][yloc]=true;
	}
